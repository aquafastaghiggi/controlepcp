...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        tbody tr.setup td { background: #fef9f0; color: #92400e; font-size: 10px; }
...
<div class="header">
    ...
    <div class="header-titulo">
        <p>Controle e Sequenciamento PCP — Sopro</p>
    </div>
    <div class="header-info">
        <div><strong>Emitido em:</strong> {{ now()->locale('pt_BR')->isoFormat('ddd DD/MM/YYYY HH:mm') }}</div>
        <div><strong>Usuário:</strong> {{ auth()->user()->name ?? 'Sistema' }}</div>
        <div><strong>Doc:</strong> #{{ $programacao->id }}</div>
    </div>
</div>
...
<div class="meta">
    <div class="meta-item">
        <span class="meta-label">Máquina</span>
        <span class="meta-value">{{ $programacao->maquina->codigo ?? '—' }} — {{ $programacao->maquina->nome ?? '—' }}</span>
    </div>
    <div class="meta-item">
...
<table>
    ...
    <tbody>
        ...
        <tr class="producao">
            ...
            <td class="right">{{ number_format((float)($item?->quantidade ?? 0) * 1000, 0, ',', '.') }}</td>
        ...
        <tr class="setup">
            ...
            <td></td>
            <td></td>
            <td style="text-align:right; font-style:italic; font-size:10px; color:#92400e;">Setup → {{ $resultado->sku }}</td>
...
{{-- TOTAIS --}}
@php $fmtMin = fn($m) => intdiv($m, 60) . 'h ' . str_pad($m % 60, 2, '0', STR_PAD_LEFT) . 'min'; @endphp
<div class="totais">
...
{{-- RODAPÉ --}}
<div class="rodape">
    <span>ControlePCP v2.0 — Aquafast · Sopro</span>
