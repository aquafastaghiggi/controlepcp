...
foreach ($argv ?? [] as $arg) {
    if (str_starts_with($arg, '--op=')) {
        $targetOp = ltrim(substr($arg, 5), '0');
        if ($targetOp === '') {
            $targetOp = '0';
        }
    }
...
for ($i = 0; $i < $rows->length; $i++) {
    ...
    $opRaw = cellValueFromRowXml($rowXml, 'AN');
...
echo "Backfill OP {$targetOp} concluido.\n";
echo 'Linhas brutas importadas: ' . $insertedEventRows . PHP_EOL;
echo 'Linhas agregadas importadas: ' . $insertedExcelRows . PHP_EOL;
