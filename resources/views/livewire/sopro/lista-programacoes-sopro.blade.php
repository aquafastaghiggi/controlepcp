<div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mb-5 shadow-sm flex flex-wrap gap-3 items-end">
        <div>
            <label class="block text-xs text-gray-500 mb-1">Máquina</label>
            <select wire:model.live="filtroMaquinaId"
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500">Máquina</th>
                            ...
                            @foreach($turnosUnicos as $turno)
                                <th class="px-2 py-3 text-center text-xs font-medium {{ $turno['noturno'] ? 'text-amber-700' : 'text-gray-500' }}">
                                    {{ $turno['noturno'] ? '🌙 ' : '' }}{{ $turno['nome'] }}
                                    <span class="block font-normal normal-case text-[11px] {{ $turno['noturno'] ? 'text-amber-600' : 'text-gray-400' }}">
                        ...
                        @foreach($programacoes as $prog)
                            ...
                            <tr wire:key="prog-{{ $prog->id }}" class="transition-colors {{ $turnoNoturno ? 'bg-amber-50 hover:bg-amber-100' : 'hover:bg-gray-50' }}">
                                ...
                                <td class="px-4 py-3 font-semibold text-gray-800">
                                    {{ $prog->maquina->codigo ?? '—' }} — {{ $prog->maquina->nome ?? '—' }}
                                </td>
                                ...
                                @foreach($turnosUnicos as $turno)
                                    @php $turnoAtivo = $grade[$turno['chave']] ?? false; @endphp
                                    <td class="px-2 py-3 text-center">
                                        @if($turnoAtivo)
                                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-semibold {{ $turno['noturno'] ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600' }}">
                                                ✓
                                            </span>
                                        @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        @if($prog->resultados_count > 0)
                                            <a href="{{ route('sopro.resultado', $prog->id) }}"
                                               class="text-xs text-blue-600 hover:underline">
                                                Ver Resultado
                                            </a>
                                            <a href="{{ route('sopro.imprimir', $prog->id) }}"
                                               target="_blank"
                                        ...
                                        @if($prog->status === 'confirmada')
                                            <button
                                                wire:click="recalcularProgramacao({{ $prog->id }})"
                                                wire:confirm="Recalcular redistribuirá todos os horários previstos das OPs desta programação. Deseja continuar?"
                                                class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded transition-colors"
                                                title="Recalcular horários previstos das OPs">
                                                🔄 Recalcular
                                            </button>
                                        @endif
                                    </div>
                            ...
                            @if($estaExpandida)
                                ...
                                <tr wire:key="prog-expand-{{ $prog->id }}">
                                    <td colspan="{{ 8 + count($turnosUnicos) }}" class="p-0 border-b border-gray-100">
                                                    ...
                                                    <table class="w-full text-xs">
                                                        <thead>
                                                            <tr class="text-gray-400 border-b border-gray-200">
                                                                ...
                                                                <th class="text-right pb-2 font-medium">Prev. frascos</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                    ...
                    <tfoot>
                        <tr class="bg-gray-50 border-t-2 border-gray-300">
                            <td colspan="{{ 7 + count($turnosUnicos) }}" class="px-4 py-3 text-right font-semibold text-gray-700">
                        ...
                        @if($totalGeralPrevCx > 0)
                            <tr class="bg-gray-50 border-t border-gray-200">
                                <td colspan="{{ 7 + count($turnosUnicos) }}" class="px-4 py-3 text-right font-semibold text-gray-600">
    ...
    <div class="mt-8">
        ...
        @foreach($historico as $maquinaId => $versoes)
        <div class="mb-4">
            <p class="text-xs font-semibold text-gray-400 uppercase mb-2">
                {{ $versoes[0]['maquina']['codigo'] ?? 'Máquina '.$maquinaId }} — {{ $versoes[0]['maquina']['nome'] ?? '' }}
            </p>
            ...
            <div class="bg-white border border-gray-200 rounded-lg mb-2 overflow-hidden">
                <div class="flex items-center px-4 py-3 hover:bg-gray-50 transition-colors">
                    <button wire:click="toggleHistorico({{ $prog['id'] }})"
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span class="text-xs text-gray-400">
                                ...
                                &middot;
                                {{ number_format(collect($prog['itens'])->sum('quantidade') * 1000, 0, ',', '.') }} frascos
                            </span>
                        </div>
                    ...
                    <table class="w-full text-xs">
                        <thead>
                            <tr class="text-gray-400 border-b border-gray-100">
                                ...
                                <th class="text-left pb-2 font-medium">Frasco</th>
