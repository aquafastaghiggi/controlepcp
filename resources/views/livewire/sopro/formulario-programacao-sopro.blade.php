<div>
    ...
    {{-- ══════════════════════════════════════════════════════════════════
         ETAPA 1 — ENTRADA
    ══════════════════════════════════════════════════════════════════ --}}
    {{-- ── Header: Upload + Arquivo + Tabs — sempre no DOM para manter ImportarExcelSopro montado.
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm mb-4">
        ...
        <div @class(['p-6' => true, 'hidden' => $excelCarregado])>
            ...
            <livewire:sopro.importar-excel-sopro />
        </div>
        ...
        @if($excelCarregado)
            ...
            {{-- Tabs de máquina com badge calculada --}}
                ...
                @foreach($abasDisponiveis as $aba)
                    ...
                    @else
                        <span class="px-3 py-1.5 text-sm text-gray-300 line-through cursor-not-allowed"
                              title="{{ $aba['nome'] }} — máquina não cadastrada">
                            {{ $aba['nome'] }}
                        </span>
                    @endif
                @endforeach
    ...
    @if($etapaAtual === 'entrada')
        ...
        @if($excelCarregado)
            {{-- wire:key força re-init do Alpine ao trocar de máquina --}}
            <div x-data="gradeDias(@js($configuracaoDias), @js($turnosDisponiveis), @js($proximosDias))"
                 wire:key="prog-grade-{{ $maquinaId }}-{{ count($turnosDisponiveis) }}">
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm mb-4">
                    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700">
                                Ordens — {{ $maquinaNome ?: $abaSelecionada }}
                            </h3>
                    ...
                    @if(count($itens) === 0)
                        <div class="px-5 py-8 text-center text-gray-400 text-sm">
                            Nenhuma OP com SKU cadastrado nesta máquina.
                        </div>
                    @else
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($itens as $item)
                                        ...
                                        <tr @class(['hover:bg-gray-50' => true, 'opacity-50' => $semSeq])>
                                            ...
                                            <td class="px-4 py-2.5 text-right font-medium tabular-nums">{{ number_format($item['quantidade'] * 1000, 0, ',', '.') }}</td>
    ...
    @if($etapaAtual === 'resultado' && count($resultados) > 0)
        ...
        @php
            ...
            foreach ($abasDisponiveis as $aba) {
                if (! $aba['maquina_existe']) continue;
        ...
        <div class="flex flex-wrap gap-3 items-center mb-2">
            @if($proximaAba)
                <button wire:click="trocarAba('{{ $proximaAba['nome'] }}')"
                    ...
                    Próxima máquina: {{ $proximaAba['maquina_codigo'] }}
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                </button>
            @elseif(count($abasCalculadas) > 0)
                <span class="text-xs text-green-600 font-medium">✓ Todas as máquinas calculadas</span>
            @endif
        </div>

        <div class="flex gap-3">
            <button wire:click="recalcular"
                    class="border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium px-5 py-2.5 rounded-lg text-sm transition-colors cursor-pointer">
                🔄 Recalcular
            </button>
...
<script>
    ...
    // ── Gantt ─────────────────────────────────────────────────────────────────
    $wire.on('gantt-sopro-atualizado', ({ resultados, turnos }) => renderizarGantt(resultados, turnos ?? {}));

    async function renderizarGantt(resultados, turnos = {}) {
        // Aguarda o locale pt-BR terminar de carregar (elimina race condition:
        // o evento gantt-sopro-atualizado pode disparar antes do import() ES module completar).
            ...
            beforeDraw(chart, _args, opts) {
                ...
                for (let d = new Date(startDay); d.getTime() < xMax; d.setDate(d.getDate() + 1)) {
                    ...
                    for (const s of sorted) {
                        ...
                        if (tsIni > cursor) fillBand(cursor, tsIni);
                        cursor = tsFim;
                    }
                    if (cursor < end24) fillBand(cursor, end24);
                }
        ...
        const pluginHachuraSemCodi = {
            ...
            afterDatasetDraw(chart, args) {
                ...
                meta.data.forEach((bar, i) => {
                    ...
                    if (comRealizado.has(pt?.y)) return;
